...
(use-package window
  ...
  (defun my/get-buffer-match-condtion (majormodes &optional buffernames &rest extra-conds)
...
the function finds the next available slot for that side."
  ...
  (defun my/display-buffer-force-right-side nil
    (interactive)
    (let ((buf (current-buffer)))
      (display-buffer buf '(nil . ((category . force-side-right))))))
  (defun my/display-buffer-force-bottom-side nil
    (interactive)
    (let ((buf (current-buffer)))
      (display-buffer buf '(nil . ((category . force-side-bottom))))))
  (defun my/display-buffer-force-top-side nil
    (interactive)
    (let ((buf (current-buffer)))
  ...
  (setq display-buffer-alist
        `(
          ;; Windows on top
          ((and (not (category . force-side-right) (category . force-side-bottom) (category . detached-side-window))
                ,(my/get-buffer-match-condtion my/top-side-window-modes-list my/top-side-window-names-list '(category . force-side-top)))
           (my/display-buffer-in-side-window-next-free-slot)
           (side . top)
           (window-height . (lambda (win) (fit-window-to-buffer win 0.5 5)))
           (dedicated . t)
           (window-parameters . (,@my/side-window-parameters
                                 (header-line-format . ,my/header-line-format-top)
                                 (mode-line-format . t))))
          ;; Windows on the right side
          ((and (not (category . force-side-top) (category . force-side-bottom) (category . detached-side-window))
                ,(my/get-buffer-match-condtion my/right-side-window-modes-list my/right-side-window-names-list '(category . force-side-right)))
          ...
          ((and (not (category . force-side-top) (category . force-side-right) (category . detached-side-window))
                ,(my/get-buffer-match-condtion my/bottom-side-window-modes-list my/bottom-side-window-names-list '(category . force-side-bottom)))
           (my/display-buffer-in-side-window-next-free-slot)
           (side . bottom)
           (preserve-size . (nil . t))
           (window-height . 20)
  ...
  (("M-o" . other-window-prefix)
   ...
   :exit
   ("R" . my/display-buffer-force-right-side)
   ("B" . my/display-buffer-force-bottom-side)
   ("T" . my/display-buffer-force-top-side)
   ("=" . balance-windows)))
