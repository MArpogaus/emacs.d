...
the source becomes invisible, and a header bar with =↗= (edit) and =✕=
(show the source) sits below the cell's =# %%= line.  The block is a
result block in every way; its overlay is one character larger, so the
invisible run starts at the end of the visible boundary line.  That
line must stay visible: an invisible run that begins at a line start
makes =scroll-down= fail with a beginning-of-buffer error.  Links keep
the keymap that shr gave them, so a click follows the URL.
...
#+begin_src emacs-lisp
  (use-package emacs
    ...
    (defun my/pycell--delete (ov)
      ...
      (dolist (prop '(my/pycell-body my/pycell-head))
    ...
    is why the =# %%= line stays visible."
      (let* ((start (1- beg))
             ...
             (bov (overlay-get ov 'my/pycell-body))
             ;; The header rides a before-string on the boundary line, so
             ;; it draws above the =# %%= comment without touching the
             ;; invisible run — which must keep starting at a line end.
             (hov (make-overlay (save-excursion (goto-char beg)
                                                (forward-line -1)
                                                (point))
                                beg)))
        ...
        (overlay-put ov 'my/pycell-head hov)
        (overlay-put hov 'evaporate t)
        (overlay-put hov 'keymap my/pycell-md-map)
