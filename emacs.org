...
#+end_src
**** my/pycell :own:
Notebook-style inline results for Python code cells.  Only =python.el=
and =comint-mime= do the work — no =emacs-jupyter=, no =zmq=.  The minor
mode =my/pycell-mode= holds everything.  It turns on automatically in
Python buffers with cells.  Turn it off to remove all blocks and to get
plain =python-shell-send-region= back.

How results arrive: =python-shell-send-string= wraps code in
=__PYTHON_EL_eval=, which returns the value of a trailing expression.
That value goes through IPython's displayhook, where comint-mime renders
rich output into the REPL.  The REPL keeps the full log.  While a cell
runs, a 0.2s ticker mirrors the rendered region — text properties,
images and all — below the cell.  When the prompt returns, a comint
output filter shows the final result.  Rich MIME output requires an
IPython REPL; a plain =python3= shell yields text only.

How blocks draw (results and markdown cells share this path): a text
body sits as a =display= property on the newline that ends the cell.
Real buffer text scrolls smoothly, and the property lives on a small
overlay, so the buffer text never changes — files, eglot and undo stay
untouched.  Display properties do not nest: a body that contains images
must ride the overlay's after-string instead, and such blocks jump when
scrolled into from below.  Two caps bound that jump: at most one figure
shows inline, and =my/pycell-max-lines= limits the text.  Every block
carries an explicit base face — an overlay string without one inherits
a face from the buffer text next to it.

The header bar shows a spinner and a stopwatch while the cell runs, then
the line count and runtime — or /no output/, so evaluated cells are
recognizable.  A cell whose interpreter dies shows =⚠= and a death
notice.  Buttons, right-aligned pixel-exactly: =▸/▾= folds, =⧉= copies,
=↗= opens the full result in a side-window buffer (one per cell), =⤓=
saves a result's image and =✕= discards.  =TAB= at the end of a cell
folds too.

Markdown cells (=# %% [markdown]=): the mode uncomments the source,
converts it with =markdown-command= and shr, and shows it in place —
the source becomes invisible, the =# %%= line with it, and a header bar
with =↗= (edit) and =✕= (show the source) takes their place.  The block
is a result block in every way; its overlay only reaches one line
further up, so the invisible run starts at the end of the visible line
above the cell — outline fold geometry.  Links keep the keymap that shr
gave them, so a click follows the URL.
=$…$= and =$$…$$= fragments become LaTeX images through org's formula
machinery, cached under =~/.cache/pycell-math= — a directory that the
LaTeX distrobox reaches, unlike =/tmp=.  Evaluate a markdown cell to
render it again.  =RET= or =mouse-2= opens the cell in a =markdown-mode=
buffer; =C-c C-c= commits the edit, =C-c C-k= discards it.
=mouse-1= reveals the plain source.  Without a =markdown-command=
processor, the cells stay plain.

Few moving parts run a cell: one buffer-local state per shell
(=my/pycell--run=), one completion filter, one ticker.  Every way a cell
ends — prompt, dead process, killed shell buffer, shell restart — goes
through the single exit =my/pycell--end=.  One cell runs at a time.
Without an interpreter, evaluation starts one and the cell follows on
its first prompt.  =C-c C-c= interrupts.  In the cells repeat map, =O=
clears all results, =0= restarts the interpreter and =R= — also on
=C-M-<return>= — restarts and runs all cells, stopping at the first
error or on =ESC=.
#+begin_src emacs-lisp
  (use-package emacs
    :ensure nil
    :custom
    ;; Without this, `line-move' vscrolls into tall display blocks and out
    ;; again, which reads as erratic jumps.
    ...
    `my/pycell-pop-output' to see the full result."
      :type 'natnum)

    ;;;; Block display, shared by results and markdown cells

    (defun my/pycell--overlays (beg end &optional prop)
      "Return the result overlays between BEG and END.
    With PROP, return the overlays that carry PROP instead."
      (let ((prop (or prop 'my/pycell)))
        (seq-filter (lambda (o) (overlay-get o prop)) (overlays-in beg end))))

    (defun my/pycell--delete (ov)
      "Delete the block overlay OV together with its body overlay."
    ...
    (defun my/pycell--button (label help command)
      "Return LABEL as a button that calls COMMAND on mouse-1.
    HELP becomes the tooltip."
      (propertize label 'mouse-face 'highlight 'help-echo help
                  'keymap (let ((map (make-sparse-keymap)))
                            (define-key map [mouse-1] command)
                            map)))

    (defun my/pycell--bar (left icons)
    ...
    (defun my/pycell--make-overlay (beg end)
    ...
    (see `my/pycell--attach'); rear-advance keeps both intact when the
    ...
    (defun my/pycell--attach (ov head body)
    ...
    (defun my/pycell--at-point (event prop)
    ...
    A click also selects its window and moves point there."
      (when-let* (((consp event))
        ...
        (goto-char pos))
      (car (my/pycell--overlays (max (1- (point)) (point-min))
    ...
    At most `my/pycell-max-lines', and nothing after the first line that
        ...
        (let* ((lines (unless (string-empty-p text) (split-string text "\n")))
          ...
          (my/pycell--attach
           ...
           (concat (unless (eq (char-before (overlay-end ov)) ?\n) "\n")
                   (my/pycell--header folded (length lines) (length shown)
                                      runtime running
    ...
    state of a replaced result is kept."
      (let ((folded (when-let* ((old (car (my/pycell--overlays beg end))))
        ...
        (let ((ov (my/pycell--make-overlay beg end)))
    ...
    (defun my/pycell--overlay (event)
      ...
      (or (my/pycell--at-point event 'my/pycell)
    ...
    (defun my/pycell-save-image (&optional event)
      "Save the first image of the result at point to a file.
    ...
    (defun my/pycell-pop-output (&optional event)
      "Show the result at point in its own buffer.
    ...
    (defun my/pycell--md-cell-p (pos)
      "Return non-nil when POS sits in the body of a markdown cell.
    The header line above the body is =# %% [markdown]=."
      (save-excursion
        (goto-char pos)
        (forward-line -1)
        (looking-at-p "# %% \\[markdown\\]")))

    (defun my/pycell--fill-props (string props)
      "Add the plist PROPS where STRING does not carry them yet.
      ...
      (when (and (require 'org nil t) (fboundp 'org-create-formula-image))
        (let* ((fg (face-attribute 'default :foreground))
          ...
          (condition-case err
            ...
            (error (message "my/pycell: LaTeX preview failed: %s"
                            (error-message-string err))
                   nil)))))

    (defun my/pycell--md-mathify (text)
    ...
    A fragment that fails to render stays plain."
      (replace-regexp-in-string
       ...
       (lambda (frag)
         ...
         (save-match-data
           (if-let* ((img (my/pycell--md-latex-image frag)))
               (propertize frag 'display img)
             frag)))
       text t t))

    (defun my/pycell--md-rendered (md)
      "Render the markdown MD to a propertized string.
    `markdown-command' produces HTML, shr renders it, and LaTeX
    fragments become preview images."
      (require 'shr)
      (let ((dom (with-temp-buffer
                   (insert md)
                   (with-current-buffer (markdown)
                     (libxml-parse-html-region (point-min) (point-max))))))
        (with-temp-buffer
          (shr-insert-document dom)
          (my/pycell--md-mathify (string-trim (buffer-string))))))

    (defvar-keymap my/pycell-md-map
      :doc "Keymap on rendered markdown cells."
      "RET" #'my/pycell-md-edit
      "<mouse-2>" #'my/pycell-md-edit
      "<mouse-1>" #'my/pycell-md-raw)

    (defun my/pycell--md-show (beg end)
      "Show the markdown cell body BEG..END rendered, in place.
    The block is built exactly like a result block — the overlay just
    reaches further up, over the cell's =# %%= line, and hides its
    text.  The invisible run therefore starts at the end of the visible
    line above the cell, the same geometry as an outline fold, and the
    block anchors on the body's final newline, never on the overlined
    boundary."
      (let* ((start (max (point-min)
                         (1- (save-excursion (goto-char beg)
                                             (forward-line -1)
                                             (point)))))
             (_ (mapc #'my/pycell--delete
                      (my/pycell--overlays start end 'my/pycell-md)))
             (help "RET/mouse-2: edit this markdown cell, mouse-1: show source")
             (text (my/pycell--fill-props
                    (my/pycell--faced
                     (my/pycell--md-rendered
                      (my/pycell--md-uncomment
                       (buffer-substring-no-properties beg end)))
                     'default)
                    (list 'keymap my/pycell-md-map 'help-echo help)))
             (head (my/pycell--bar
                    " markdown"
                    (concat (my/pycell--button
                             ...
                             #'my/pycell-md-edit)
                            "  "
                            (my/pycell--button "✕" "Show the plain source"
                                               #'my/pycell-md-raw)
                            " ")))
             (ov (my/pycell--make-overlay start end))
             (bov (overlay-get ov 'my/pycell-body)))
        ;; The property carries the body bounds: the overlay starts
        ;; above the cell and ends before the final newline.
        (overlay-put ov 'my/pycell-md
                     (cons (copy-marker beg) (copy-marker end t)))
        (overlay-put ov 'invisible t)
        (overlay-put ov 'keymap my/pycell-md-map)
        (when bov
          (overlay-put bov 'keymap my/pycell-md-map)
          (overlay-put bov 'help-echo help))
        ;; The leading newline breaks the line above the cell; at the
        ;; start of the buffer there is none to break.
        (my/pycell--attach
         ov (concat (unless (= start (point-min)) "\n") head) text)))

    (defun my/pycell-md-render-all ()
      "Render every markdown cell (=# %% [markdown]=) in the buffer."
      (interactive)
      (require 'markdown-mode)
      (let ((exe (executable-find (car (split-string markdown-command))))
            missing)
        (save-excursion
          (goto-char (point-min))
          (while (re-search-forward "^# %% \\[markdown\\]" nil t)
            ...
            (let ((beg (point))
                  (end (if (re-search-forward code-cells-boundary-regexp nil t)
                         ...
                         (point-max))))
              (when (< beg end)
                (if exe (my/pycell--md-show beg end) (setq missing t)))
              (goto-char end))))
        (when missing
          (message "my/pycell: `%s' not found, markdown cells stay plain"
                   markdown-command))))

    (defun my/pycell-md-unrender ()
      "Show all markdown cells as their plain source again."
      (interactive)
      (mapc #'my/pycell--delete
            (my/pycell--overlays (point-min) (point-max) 'my/pycell-md)))

    (defun my/pycell-md-raw (&optional event)
      "Show the markdown cell at point as plain source.
    The cell is then editable in place; evaluate it, or run
    `my/pycell-md-render-all', to render it again."
      (interactive (list last-input-event))
      (my/pycell--delete
       (or (my/pycell--at-point event 'my/pycell-md)
           (user-error "No rendered markdown cell here"))))

    (defvar-local my/pycell--md-source nil
      "Markdown cell (BUFFER BEG END) that this edit buffer feeds.")

    (define-minor-mode my/pycell-md-edit-mode
      ...
      :keymap (define-keymap
                "C-c C-c" #'my/pycell-md-commit
                "C-c C-k" #'my/pycell-md-abort))

    (defun my/pycell-md-edit (&optional event)
      "Edit the markdown cell at point in a `markdown-mode' buffer.
    The body opens without its comment prefixes.
    \\<my/pycell-md-edit-mode-map>\\[my/pycell-md-commit] puts it back \
    and renders it; \\[my/pycell-md-abort] discards the edit."
      (interactive (list last-input-event))
      (pcase-let* ((ov (or (my/pycell--at-point event 'my/pycell-md)
                           (user-error "No rendered markdown cell here")))
                   (`(,beg . ,end) (overlay-get ov 'my/pycell-md))
                   (src (current-buffer))
                   (md (my/pycell--md-uncomment
                        (buffer-substring-no-properties beg end)))
        ...
        (with-current-buffer buf
          ...
          (insert md)
          (markdown-mode)
          (my/pycell-md-edit-mode)
          ;; With standard-keys-mode, the C-c prefix sits behind C-p;
          ;; show both routes.
          (setq header-line-format
                " Markdown cell — apply: C-p C-c (C-c C-c), discard: C-p C-k")
          (setq my/pycell--md-source (list src beg end)))
        (pop-to-buffer buf)))

    (defun my/pycell-md-commit ()
      "Put the edited markdown back into its cell and render it."
      (interactive)
      (pcase-let ((`(,src ,beg ,end) my/pycell--md-source)
        ...
        (with-current-buffer src
          (save-excursion
            ...
            (insert (my/pycell--md-comment md) "\n"))
          (my/pycell--md-show beg end))
        (quit-window t)))

    (defun my/pycell-md-abort ()
    ...
    (defvar my/pycell--queue nil
      "Start markers of the cells that `my/pycell-restart-and-run-all' still runs.")
    ...
    Unlike `my/pycell--run-next' this does not move point: the command
    ...
    first prompt."
      ...
      (if (my/pycell--md-cell-p start)
          ;; Keep a running restart-and-run-all chain going — no prompt
          ;; will arrive to do it.
          (progn
            (my/pycell--md-show start end)
            (when my/pycell--queue (my/pycell--run-next)))
    ...
    \\<my/pycell-stop-map>\\[my/pycell-stop]."
      (interactive)
      (my/pycell-restart)
      ...
      ;; hence the depth.  `my/pycell--end' chains the remaining cells.
    ...
    `my/pycell-eval-region'.  Turn it off to remove all blocks and to
    ...
    the code-cells maps."
      ...
      :lighter " pycell"
      (if my/pycell-mode
          (my/pycell-md-render-all)
        (my/pycell-remove-overlays)
        (my/pycell-md-unrender)))
    ...
    :config
    ;; Keyed on the minor mode: with it off, code-cells falls through to
    ;; its stock python entry, `python-shell-send-region'.
    ...
    (with-eval-after-load 'auto-side-windows
      ...
      (add-to-list 'auto-side-windows-right-buffer-names "^\\*pycell md: .*\\*$")))
#+end_src
