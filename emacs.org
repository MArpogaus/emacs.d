...
#+begin_src emacs-lisp
  (use-package tab-bar
    ...
    (defun my/tab-bar--format-tab-group-around (orig-fun tab i &rest rest)
      (let ((color (face-attribute (if (eq (car tab) 'current-tab)
                                       'mode-line-emphasis
                                     'tab-bar-tab-group-inactive) :foreground)))
        (cons `(,(intern (format "group-sep-%i" i)) menu-item
                ,(propertize " " 'face (list :height 0.2
                                             :foreground color
                                             :background color))
                ignore)
              (funcall orig-fun tab i rest))))
