...
- [[#faster-fine-grained-code-highlighting][Faster, fine-grained code highlighting.]]
- [[#more-flexible-code-folding][More flexible code folding.]]
- [[#structural-editing-like-paredit-or-even-better-for-non-lisp-code][Structural editing (like Paredit, or even better) for non-Lisp code.]]
- [[#more-informative-indexing-for-imenu][More informative indexing for imenu.]]
...
#+begin_src emacs-lisp :tangle no
  (use-package dashboard
    ...
    :hook
    (init-hook . dashboard-setup-startup-hook))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (use-package tramp-sh                                     
    :straight nil                                             
    :config
    ;; TRAMP does not normally retain this remote path after login.
    ;; However, tramp-own-remote-path preserves the path value, which can be used to update tramp-remote-path.
...
#+begin_src emacs-lisp
  (use-package tab-bar
    ...
    (defun my/tab-bar-tab-name-format-around (orig-fun tab i)
      (let ((color (face-attribute (if (eq (car tab) 'current-tab)
                                       'mode-line-emphasis
                                     'mode-line-inactive) :foreground)))
        (concat
         (propertize "M"
                     'face (list :height 0.3
                                 :foreground color
                                 :background color
                                 :box color))
         (funcall orig-fun tab i))))

    (advice-add 'tab-bar-tab-name-format-default :around #'my/tab-bar-tab-name-format-around)
    :hook
