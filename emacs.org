...
#+begin_src emacs-lisp
  (use-package emacs
    ...
    With PROP, return the overlays that carry PROP instead."
      (seq-filter (lambda (o) (overlay-get o (or prop 'my/pycell)))
                  (overlays-in beg end)))
    ...
    (defun my/pycell-remove-overlays (&optional beg end prop)
    ...
    (defun my/pycell--icons (&rest buttons)
    ...
    while the cell runs.  IMAGEP marks a result with an image."
      (let* ((icons (my/pycell--icons
                     (when imagep
                       (my/pycell--button "⤓" "Save the result's image to a file"
                                          #'my/pycell-save-image))
                     (when (> total 0)
                       (my/pycell--button "⧉" "Copy this result"
                                          #'my/pycell-copy-output))
                     (when (> total 0)
                       (my/pycell--button "↗" "Show this result in its own buffer"
    ...
    (defun my/pycell--fill-prop (string prop value)
    ...
    is why the =# %%= line stays visible."
      (let* ((start (1- beg))
             (_ (my/pycell-remove-overlays start end 'my/pycell-md))
             (help "RET/mouse-2: edit this markdown cell, mouse-1: show source")
             (text (my/pycell--fill-prop
                    (my/pycell--fill-prop
                     (my/pycell--faced
             ...
             (hov (make-overlay (my/pycell--md-head beg) beg)))
        ...
        (overlay-put hov 'my/pycell-md t)
        (overlay-put hov 'my/pycell-main ov)
    ...
    (defun my/pycell-md-unrender ()
      ...
      (interactive)
      (my/pycell-remove-overlays (point-min) (point-max) 'my/pycell-md))

    (defun my/pycell--md-at (event)
    ...
    small overlay that draws it."
      (let ((ov (or (my/pycell--at-point event 'my/pycell-md)
                    (user-error "No rendered markdown cell here"))))
        (or (overlay-get ov 'my/pycell-main) ov)))
    ...
    `my/pycell-md-render-all', to render it again."
      (interactive (list last-input-event))
      (my/pycell--delete (my/pycell--md-at event)))
    ...
    and renders it; \\[my/pycell-md-abort] discards the edit."
      (interactive (list last-input-event))
      (pcase-let* ((ov (my/pycell--md-at event))
        ...
        (with-current-buffer buf
          ...
          ;; The keys come from the keymap, so the hint stays true under
          ;; `standard-keys-mode', which moves the C-c prefix to C-p.
          (setq header-line-format
                (substitute-command-keys
                 " Markdown cell — \\[my/pycell-md-commit] applies, \
      \\[my/pycell-md-abort] discards"))
