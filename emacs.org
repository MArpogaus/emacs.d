...
height cap keeps that bounded.
Evaluation never blocks Emacs, but one cell at a time: sending while
another cell is still running is refused.  In the cells repeat map, =O=
...
#+begin_src emacs-lisp
  (use-package emacs
    ...
    (defun my/pycell--header (folded lines)
      "Return the header bar of a FOLDED or unfolded result of LINES lines."
      (let ((icons (concat (my/pycell--button "⧉" "Show this result in its own buffer"
                           ...
                           " ")))
        (my/pycell--faced
         (concat (my/pycell--button (if folded " ▸" " ▾") "Fold or unfold this result"
                                    #'my/pycell-toggle-output)
                 (format " %d line%s%s" lines (if (= lines 1) "" "s")
                         (if (or folded (<= lines my/pycell-max-lines)) ""
                           (format ", showing %d" my/pycell-max-lines)))
                 ;; Stretch space, so the icons end up at the window's right edge.
                 ;; Two columns of slack: the last one can be reserved for the
                 ;; continuation glyph, and arrows may render wider than
                 ;; `string-width' claims — either one wraps the line.
                 (propertize " " 'display
                             `(space :align-to (- right ,(+ 2 (string-width icons)))))
                 icons)
         'my/pycell-header)))
    ...
    contains an image."
      (pcase-let ((`(,folded ,text) (overlay-get ov 'my/pycell))
                  (end (overlay-end ov)))
        (let* ((lines (split-string text "\n"))
               (header (my/pycell--header folded (length lines)))
               (body (unless folded
                       (my/pycell--faced
                        (string-join (seq-take lines my/pycell-max-lines) "\n")
                        'my/pycell-output)))
    ...
    (defun my/pycell--show (beg end text)
      ...
      (unless (string-empty-p text)
        ...
        (let* ((tip (if (and (eq (char-before end) ?\n) (> (1- end) beg))
          ...
          (overlay-put ov 'my/pycell (list nil text))
    ...
    (defvar-local my/pycell--target nil
      "State of the cell running in this Python shell: (FROM BEG END TAIL).
    FROM marks where its output starts here, BEG and END delimit the cell in
    its own buffer, TAIL accumulates the last output chunks.")

    (defun my/pycell--filter (output)
      "Mirror the running cell's OUTPUT below it, once the prompt is back.
    Runs in the Python shell, from `comint-output-filter-functions'."
      (when my/pycell--target
        (pcase-let ((`(,from ,beg ,fin ,tail) my/pycell--target))
          ...
          (when (python-shell-comint-end-of-output-p tail)
            ...
            (let ((text (my/pycell--clean
                         (buffer-substring from (if comint-last-prompt
                                                    (car comint-last-prompt)
                                                  (point-max))))))
              (when (buffer-live-p (marker-buffer beg))
                (with-current-buffer (marker-buffer beg)
                  (my/pycell--show beg fin text)))
      ...
      (let ((proc (python-shell-get-process-or-error))
        ...
        (with-current-buffer (process-buffer proc)
          ...
          (setq my/pycell--target
                (list (copy-marker (process-mark proc)) beg fin ""))
