...
#+begin_src emacs-lisp
  (use-package auto-side-windows
      ...
      '(:eval
        (let*
            ((buffer (current-buffer))
             ...
             (buttons (concat
                       ...
                       (my/side-window-button "✕" "Close this side window and kill its buffers"
                                              #'my/side-window-kill))))
          ;; The tail below aligns to the right edge of the text area and
          ;; then runs on, so it needs a right margin to land in; window-box
          ;; draws the sides from the fringes and leaves the margins alone.
          ...
          (list
           ...
           buttons
           " "
           ;; Two pixels wide, so the cap lines up with the box edge
           ;; window-box draws down the sides of the body.
           (propertize " " 'face prefix-face 'display '(space :width (2)))))))
    :custom
    ...
    (auto-side-windows-bottom-window-parameters `((mode-line-format . none)
                                                  (header-line-format . ,my/header-line-format-top)))
    ;; window-box draws the frame around every side window; detaching a
    ;; buffer into a normal window takes the box away, and displaying it
    ;; on a side brings it back.
    (auto-side-windows-after-display-hook '((lambda (buffer &rest _)
                                              (with-current-buffer buffer
                                                (window-box-mode 1)))))
    (auto-side-windows-before-toggle-hook '((lambda (buffer)
                                              (with-current-buffer buffer
                                                (window-box-mode -1)))))
