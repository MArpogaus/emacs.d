...
through any of its commands or via =SPC j=, single keys (=n p e r a b o O
0 R=) then chain until a foreign key exits.  No prefix bindings, and no
plain speed keys, which meow's normal state would shadow.  Rarely used
commands (split, merge, markdown cells) are =M-x= only.  Cell results are
rendered inline by =my/pycell= below.
#+begin_src emacs-lisp
  (use-package code-cells
    ...
    (:map code-cells-mode-map
          ...
          :repeat-map my/code-cells-repeat-map
...
overlay strings, so long results live in the popup buffer instead; for the
same reason image height is capped through =comint-mime-image-props=.
A text body is rendered as a =display= property on the newline ending the
cell's last code line: display properties on real buffer text are what
redisplay and =pixel-scroll-precision-mode= scroll smoothly (the REPL is
fluid for exactly this reason), whereas tall after-strings made upward
scrolling jump.  The buffer's text itself is untouched — properties are
never saved, so files, eglot and undo are unaffected.  Display properties
do not nest, however, so the header (its right-aligning stretch space)
and bodies containing images stay in the overlay's after-string; figure
results consequently still jump when scrolled past from below.  The image
height cap keeps that bounded, and at most /one/ figure is drawn inline —
further ones (and everything past =my/pycell-max-lines=) live in the =↗=
popup buffer.
...
kill ring, and with =my/pycell-quiet-repl= set, output is removed from
the REPL once the cell finished — the REPL keeps only its prompts.  Evaluation never blocks
...
#+begin_src emacs-lisp
  (use-package emacs
    ...
    (defun my/pycell--header (folded total shown runtime running)
      "Return the result header bar.
    TOTAL and SHOWN count the result's lines and the inline-drawn subset,
    RUNTIME is the evaluation time in seconds so far and RUNNING non-nil
    while the cell is still evaluating."
      (let* ((icons (concat (when (> total 0)
             ...
             (state (cond (running (string ?\s (aref "⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏"
                                                     (mod (truncate (or runtime 0) 0.2)
                                                          10))))
                          ((> total 1)
             ...
             (time (when runtime (format "%.1fs" runtime))))
        (my/pycell--faced
         (concat state " "
                 (mapconcat #'identity (delq nil (list label time)) " · ")
    ...
    Call with the overlay's buffer current.  A text body is attached as a
    `display' property on the newline ending the cell's last code line —
    real buffer text, which redisplay scrolls smoothly, in contrast to tall
    overlay after-strings (the REPL is fluid for the same reason).  Display
    properties do not nest, though: inside a `display' string, images and
    the header's :align-to space would be ignored.  The one-line header
    therefore always stays in the after-string, and so does a body that
    contains an image."
        ...
        (let* ((lines (unless (string-empty-p text) (split-string text "\n")))
          ...
          (overlay-put ov 'after-string
                       (concat (unless (eq (char-before end) ?\n) "\n")
                               header
                               (when image (concat "\n" body))))
          ;; `my/pycell--show' guarantees this newline; it may only go
          ;; missing through an edit right behind the cell.
          (when (eq (char-after end) ?\n)
            (with-silent-modifications
              (if (and body (not image))
                  ;; Replacing the newline, the body reproduces the line
                  ;; breaks on both of its sides.
                  (put-text-property end (1+ end) 'display
                                     (concat "\n" body "\n"))
                (remove-text-properties end (1+ end) '(display nil))))))))

    (defun my/pycell--delete (ov)
      "Delete result overlay OV together with its displayed block."
      (let ((end (overlay-end ov)))
        (when (and end (eq (char-after end) ?\n))
          (with-silent-modifications
            (remove-text-properties end (1+ end) '(display nil)))))
      (delete-overlay ov))
    ...
    (defun my/pycell--show (beg end text &optional runtime running)
    ...
    labelled \"no output\", so evaluated cells are recognizable as such."
      ...
      (let* ((tip (if (and (eq (char-before end) ?\n) (> (1- end) beg))
             ...
             (ov (make-overlay beg tip nil t)))
        (overlay-put ov 'my/pycell (list nil text runtime running))
    ...
    (defvar my/pycell--ticker nil
      "Timer driving the stopwatch of the running cell.")

    (defun my/pycell--live-show ()
      "Refresh the running cell's overlay with the output received so far.
    Call in the Python shell buffer."
      (pcase-let ((`(,from ,beg ,fin ,_ ,start) my/pycell--target))
        ;; Strip a trailing, still incomplete escape sequence: comint-mime
        ...
        (let ((text (my/pycell--clean
          ...
          (when (buffer-live-p (marker-buffer beg))
            (with-current-buffer (marker-buffer beg)
              (my/pycell--show beg fin text (- (float-time) start) t))))))

    (defun my/pycell--tick (buf)
      "Update the running cell's stopwatch; self-cancels when BUF is done."
    ...
    Matches the calling convention of `code-cells-eval-region-commands'."
      ...
      (let ((proc (python-shell-get-process-or-error))
        ...
        (when (timerp my/pycell--ticker) (cancel-timer my/pycell--ticker))
        (setq my/pycell--ticker
              (run-with-timer 0.2 0.2 #'my/pycell--tick (process-buffer proc)))
    ...
    :config
    ;; ;; An image is a single, very tall line, which the line cap cannot bound;
    ;; ;; limit its height instead (also applies in the REPL).  250px keeps the
    ;; ;; scroll-past jump at roughly a dozen text lines.
    ;; (with-eval-after-load 'comint-mime
    ;;   (setq comint-mime-image-props '(:max-height 250)))
