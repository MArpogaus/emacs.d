  ...
  (use-package tab-bar
    ...
    (defun my/tab-bar-tab-group-format-function (tab i &optional current-p)
      (let*((tab-group-name (funcall tab-bar-tab-group-function tab))
            ...
            (icon-face (if current-p 'tab-bar-tab-group-icon-current 'tab-bar-tab-group-icon-inactive))
            (color (face-attribute (if current-p
                                       'mode-line-emphasis
                                     'tab-bar-tab-group-inactive) :foreground))
            (group-sep (concat (propertize " " 'face (list :height (if current-p 0.3 0.2)
                                                           :foreground color
                                                           :background color))
                               (propertize " " 'face tab-group-face)))
  ...
  (use-package tab-line
    ...
    (defun my/tab-line-tab-name-function (buffer &optional _buffers)
      (let ((name (buffer-name buffer)))
        (concat " "
                (propertize (nerd-icons-icon-for-file name) 'face 'tab-bar-tab-group-icon-current)
                (format " %s " name))))
    (defun my/tab-line-tab-name-format-default-around (orig-fun tab tabs)
      (let ((formatted-tab (funcall orig-fun tab tabs)))
        (put-text-property 1 2 'face
                           `((t :inherit ,(get-text-property 0 'face formatted-tab ) :family "Symbols Nerd Font Mono"))
                           formatted-tab)
        formatted-tab))
    :config
    (setq tab-line-tab-name-function #'my/tab-line-tab-name-function
          tab-line-separator "")
    (advice-add 'tab-line-tab-name-format-default :around #'my/tab-line-tab-name-format-default-around)
    :hook
...
#+begin_src emacs-lisp
  (use-package flyspell
    ...
    :hook
    (((text-mode conf-mode org-mode LaTeX-mode) . flyspell-mode)
