...
machinery, cached under =~/.cache/pycell-math= — a directory a LaTeX in
a container reaches, unlike =/tmp=.  Evaluate a markdown cell to
...
#+begin_src emacs-lisp
  (use-package emacs
    ...
    ;; Personal window placement, not part of the feature.
    (with-eval-after-load 'auto-side-windows
