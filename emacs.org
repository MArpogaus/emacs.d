...
- *Optimized UX:* Several build-in packages like =autorevert=, =del-sel=, =elec-pair=, =hide-show-mode=, =outline-mode=, =saveplace= and =savehist= are enabled and configured for optimal user experience. Additionally, third-party packages like [[https://github.com/MArpogaus/auto-side-windows.git][auto-side-window]] and [[https://github.com/karthink/popper.git][popper]] are used for a more streamlined window layout and management, while [[https://github.com/magnars/multiple-cursors.el.git][multiple-cursors]] and [[https://github.com/mhayashi1120/Emacs-wgrep.git][wgrep]] enhance the editing experience across multiple lines or files.
      ...
      '(:eval
        (let*
          ...
          (list
           ...
           ;; `string-width' merely counts columns.  In the display spec,
           ;; `right' and `-' belong to the spec language (hence outside
           ;; the unquote), and (N) denotes N pixels.
           ;; Measuring the buttons without their trailing space shifts
           ;; the tail one space into the right margin, flush with the
           ;; window edge.
           (propertize " " 'display
                       `(space :align-to
...
are =M-x= only.  Cell results are rendered inline by =my/pycell= below.
#+begin_src emacs-lisp
  (use-package code-cells
    ...
    (:map code-cells-mode-map
          ...
          ("C-M-<return>" . my/pycell-restart-and-run-all)
          ;; navigation
          ...
          ("c" . my/pycell-copy-output)
          ("i" . my/pycell-interrupt)
          ("o" . my/pycell-toggle-output)
          ("O" . my/pycell-remove-overlays)
          ("0" . my/pycell-restart)
          ("R" . my/pycell-restart-and-run-all))
    :hook
    (python-base-mode . code-cells-mode-maybe))
#+end_src
**** my/pycell :own:
Notebook-style inline results for Python code cells, built only from
=python.el= and =comint-mime= — no =emacs-jupyter=, no =zmq=.  Everything
lives in the minor mode =my/pycell-mode=, enabled automatically in Python
buffers with cells; toggling it off removes all results and reverts cell
evaluation to plain =python-shell-send-region=.
=python-shell-send-string= wraps code in =__PYTHON_EL_eval=, which returns
the value of a trailing expression; since that call runs at the REPL top
level, the value goes through IPython's displayhook, where =comint-mime=
has registered its =mime_renderers= and enabled the inline matplotlib
backend.  Output therefore flows into the REPL as usual, where comint-mime
renders it — the =*Python*= buffer keeps the complete log.  We only mirror
it: a marker at =process-mark= before sending, and once the next prompt
arrives the region up to it is copied out of the REPL /with/ its text
properties, images and all, into an overlay below the cell.  Rendering the
escape sequences ourselves is thus unnecessary, and nothing is suppressed.

The result gets a header bar with the line count and three clickable
icons, aligned right: =▾= folds, =↗= opens the full result in its own
buffer — one per cell, displayed in a right side window — and =✕=
discards it.  =TAB= at the end of the cell folds as well, =o= in the
cells repeat map from anywhere.  Only =my/pycell-max-lines= lines are drawn
inline — redisplay grows sluggish and scrolling stutters over very large
overlay strings, so long results live in the popup buffer instead.
A text body is rendered as a =display= property over the newline ending the
cell's last code line: display properties anchored to real buffer text are
what redisplay and =pixel-scroll-precision-mode= scroll smoothly (the REPL
is fluid for exactly this reason), whereas tall after-strings made upward
scrolling jump.  The property lives on a small dedicated overlay, not on
the text itself, so the buffer's text is untouched and killing or copying
the line never captures the result.  Display properties do not nest,
however, so the header (its right-aligning stretch space) and bodies
containing images stay in the overlay's after-string; figure results
consequently still jump when scrolled past from below, and at most /one/
figure is drawn inline to bound that — further ones (and everything past
=my/pycell-max-lines=) live in the =↗= popup buffer.
Output streams into the overlay live, as it arrives in the REPL, and the
header carries a spinner and stopwatch while the cell runs; the runtime
remains afterwards.  A cell without output keeps a bare header saying /no
output/, so it is recognizable as evaluated.  =⧉= copies a result to the
kill ring, =⤓= — offered when the result contains an image — saves that
image to a file (the type is taken from the image descriptor, which
`create-image' derived from the data's magic bytes), and with
=my/pycell-quiet-repl= set, output is removed from the REPL once the
cell finished — the REPL keeps only its prompts.  Evaluation never blocks
Emacs, but one cell at a time: sending while another cell is still
running is refused.  In the cells repeat map, =O=
clears all results, =0= additionally restarts the interpreter, and =R= —
also directly on =C-M-<return>= — is Jupyter's /restart kernel and run
all cells/: it re-evaluates every cell in order, stopping at the first
error or on =ESC=.  Rich MIME output requires an IPython REPL (see the
python section below); a plain =python3= shell yields text only.
#+begin_src emacs-lisp
  (use-package emacs
    :ensure nil
    :custom
    ;; `line-move' otherwise vscrolls part-way into a tall overlay string and
    ;; back out again, which reads as erratic jumping.  With nil, point steps
    ;; past the result in one go.
    (auto-window-vscroll nil)
    :preface
    (defgroup my/pycell nil "Inline results for Python code cells." :group 'python)

    (defface my/pycell-header '((t :inherit code-cells-header-line))
      "Face for the header bar above a result.
    Inherits the cell boundary face, so results visually match the cells.")

    (defface my/pycell-output '((t :inherit shadow :extend t))
      "Face for the body of a result.")

    (defcustom my/pycell-max-lines 12
      "Number of result lines drawn inline; the rest is only counted.
    Large overlay strings make redisplay and scrolling sluggish, hence the cap.
    Use `my/pycell-pop-output' to see the whole result."
      :type 'natnum)

    (defcustom my/pycell-quiet-repl nil
      "When non-nil, remove a cell's output from the REPL once it finished.
    The REPL then keeps only its prompts; results live below their cells
    alone.  While a cell runs its output still passes through the REPL —
    comint-mime must render it there before it can be mirrored."
      :type 'boolean)

    (defun my/pycell--clean (text)
      "Strip prompts, Out[n] markers and surrounding whitespace from TEXT.
    A prompt can end up at either end of the mirrored region, and IPython
    labels the value of the last expression.  Whitespace is only removed
    where it carries no display property — comint-mime renders an image as
    a single space with one, so `string-trim' would delete it.  Call this
    in the shell buffer, where `comint-prompt-regexp' is set."
      (let ((rx (concat "\\(?:" comint-prompt-regexp "\\)")))
        ;; The (> ...) guard prevents an endless loop should the prompt
        ;; regexp ever match the empty string.
        (while (and (string-match (concat "\\`[ \t\n]*" rx) text)
                    (> (match-end 0) 0))
          (setq text (substring text (match-end 0))))
        (while (string-match (concat "\n[ \t]*" rx "[ \t\n]*\\'") text)
          (setq text (substring text 0 (match-beginning 0)))))
      ;; Retained parts keep their properties, so images survive all this.
    ...
    (defun my/pycell--overlays (beg end)
      "Return all result overlays between BEG and END."
      (seq-filter (lambda (o) (overlay-get o 'my/pycell)) (overlays-in beg end)))

    (defun my/pycell-remove-overlays (&optional beg end)
      "Remove result overlays between BEG and END (default: whole buffer)."
      (interactive)
      (without-restriction
        (mapc #'my/pycell--delete
              (my/pycell--overlays (or beg (point-min)) (or end (point-max))))))

    (defun my/pycell--faced (string face)
      "Add FACE below the faces STRING already carries, and return STRING."
      (add-face-text-property 0 (length string) face t string)
      string)

    (defun my/pycell--button (label help command)
      "Return LABEL as text calling COMMAND on `mouse-1', documented by HELP."
      (propertize label 'mouse-face 'highlight 'help-echo help
                  'keymap (let ((map (make-sparse-keymap)))
                            (define-key map [mouse-1] command)
                            map)))

    (defun my/pycell--body-lines (lines)
      "Return the leading LINES to draw inline.
    At most `my/pycell-max-lines', and nothing beyond the first line
    carrying an image: several inline figures would grow the block — and
    the jump when scrolling past it — without bound."
      (let (shown stop)
        (while (and lines (not stop) (< (length shown) my/pycell-max-lines))
          (let ((l (pop lines)))
            (push l shown)
    ...
    (defun my/pycell--header (folded total shown runtime running imagep)
      "Return the result header bar.
    TOTAL and SHOWN count the result's lines and the inline-drawn subset,
    RUNTIME is the evaluation time in seconds so far, RUNNING non-nil
    while the cell is still evaluating and IMAGEP when the result
    contains an image."
      (let* ((icons (concat (when imagep
                              (concat (my/pycell--button
                                       "⤓" "Save the result's image to a file"
                                       #'my/pycell-save-image)
                                      "  "))
                            (when (> total 0)
                              (concat (my/pycell--button
                                       "⧉" "Copy this result"
                                       #'my/pycell-copy-output)
                                      "  "
                                      (my/pycell--button
                                       "↗" "Show this result in its own buffer"
                                       #'my/pycell-pop-output)
                                      "  "))
                            (my/pycell--button "✕" "Discard this result"
                                               #'my/pycell-discard-output)
                            " "))
             ;; The stopwatch drives the spinner: one frame per tick.
             (state (cond ((eq running t)
                           (string ?\s (aref "⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏"
                                             (mod (truncate runtime 0.2)
                                                  10))))
                          ((eq running 'died) " ⚠")
                          ((> total 1)
                           (my/pycell--button (if folded " ▸" " ▾")
                                              "Fold or unfold this result"
                                              #'my/pycell-toggle-output))
                          ((zerop total) " ✓")
                          (t " ")))
             (label (cond ((> total 0)
        ...
        (my/pycell--faced
         (concat state " "
                 (string-join (delq nil (list label time)) " · ")
                 ;; Pixel-exact right alignment: glyph widths differ from what
                 ;; `string-width' counts; (N) in the spec means N pixels.
                 (propertize " " 'display
                             `(space :align-to
                                     (- right (,(string-pixel-width
    ...
    (defun my/pycell--update (ov)
      "Refresh the displayed result of overlay OV.
    Call with the overlay's buffer current.  A text body is shown as a
    `display' property over the newline ending the cell's last code line —
    anchored to real buffer text, which redisplay scrolls smoothly, in
    contrast to tall overlay after-strings (the REPL is fluid for the same
    reason).  The property lives on a dedicated one-character overlay, not
    on the text itself, so killing or copying the line never captures it.
    Display properties do not nest, though: inside a `display' string,
    images and the header's :align-to space would be ignored.  The one-line
    header therefore always stays in the after-string, and so does a body
    that contains an image."
      (pcase-let ((`(,folded ,text ,runtime ,running) (overlay-get ov 'my/pycell))
                  (end (overlay-end ov))
                  (bov (overlay-get ov 'my/pycell-body)))
        (let* ((lines (unless (string-empty-p text) (split-string text "\n")))
               (shown (my/pycell--body-lines lines))
               ;; The full text, not the shown subset: an image beyond the
               ;; inline cap must still offer its save button.
               (header (my/pycell--header folded (length lines) (length shown)
                                          runtime running
                                          (and lines (my/pycell--image text))))
               (body (when (and shown (not folded))
                       (my/pycell--faced (string-join shown "\n")
                                         'my/pycell-output)))
               (image (and body (text-property-not-all
                                 0 (length body) 'display nil body))))
          (overlay-put ov 'after-string
                       (concat (unless (eq (char-before end) ?\n) "\n")
                               header
                               (when image (concat "\n" body))))
          (when (and bov (overlay-buffer bov))
            ;; Replacing the newline, the body reproduces the line breaks
            ;; on both of its sides; nil restores the plain newline.
            (overlay-put bov 'display (and body (not image)
                                           (concat "\n" body "\n")))))))

    (defun my/pycell--delete (ov)
      "Delete result overlay OV together with its body overlay."
      (when-let* ((bov (overlay-get ov 'my/pycell-body)))
        (delete-overlay bov))
      (delete-overlay ov))

    (defun my/pycell--tab-filter (cmd)
      "Return CMD when point sits at the very end of a cell showing a result."
      (and (eolp)
           (seq-some (lambda (o) (eq (point) (overlay-end o)))
                     (my/pycell--overlays (max (1- (point)) (point-min))
                                          (point)))
           cmd))

    (defvar-keymap my/pycell-overlay-map
      :doc "Keymap active inside a cell that shows a result."
      "TAB" '(menu-item "" my/pycell-toggle-output :filter my/pycell--tab-filter))

    (defun my/pycell--show (beg end text runtime &optional running)
      "Show TEXT as the result of the cell between BEG and END.
    RUNTIME is the evaluation time in seconds so far, RUNNING non-nil while
    the cell is still evaluating.  An empty TEXT still gets a header line,
    labelled \"no output\", so evaluated cells are recognizable as such.
    The fold state of a result already shown for this cell is kept."
      (let ((folded (when-let* ((old (car (my/pycell--overlays beg end))))
                      (car (overlay-get old 'my/pycell)))))
        (my/pycell-remove-overlays beg end)
        ;; The newline ending the cell carries the result (see
        ;; `my/pycell--update'); give the buffer's last cell one.
        (when (and (= end (point-max)) (not (eq (char-before end) ?\n)))
          (save-excursion (goto-char end) (insert "\n")))
        ;; End the overlay before that newline: this anchoring keeps the
        ;; result out of view when a window starts at the next cell's
        ;; boundary line.  Rear-advance, so typing at the end of the last
        ;; code line keeps the overlay's end on the (shifted) newline.
        (let* ((tip (if (and (eq (char-before end) ?\n) (> (1- end) beg))
                        (1- end)
                      end))
               (ov (make-overlay beg tip nil t t)))
          (overlay-put ov 'my/pycell (list folded text runtime running))
          (overlay-put ov 'evaporate t)
          (overlay-put ov 'keymap my/pycell-overlay-map)
          (overlay-put ov 'modification-hooks
                       (list (lambda (o &rest _) (my/pycell--delete o))))
          ;; The body overlay covers the newline; front-advance keeps it
          ;; glued to it, evaporate removes it — and with it the display
          ;; property — when the newline goes.
          (when (eq (char-after tip) ?\n)
            (overlay-put ov 'my/pycell-body
    ...
    (defun my/pycell--overlay (event)
      "Return the result overlay clicked in EVENT, or the one of the cell at point."
      (when-let* (((consp event))         ; a click: act where it landed
                  (posn (event-start event))
                  (pos (posn-point posn)))
        (select-window (posn-window posn))
        (goto-char pos))
      (or (car (my/pycell--overlays (max (point-min) (1- (point))) (point)))
          (car (apply #'my/pycell--overlays (code-cells--bounds)))
          (user-error "No result here")))

    (defun my/pycell-toggle-output (&optional event)
      "Fold or unfold the result of the cell at point, or the one clicked in EVENT."
      (interactive (list last-input-event))
      (let* ((ov (my/pycell--overlay event))
             (state (overlay-get ov 'my/pycell)))
        (setcar state (not (car state)))
        (my/pycell--update ov)))

    (defun my/pycell-discard-output (&optional event)
      "Discard the result of the cell at point, or the one clicked in EVENT."
      (interactive (list last-input-event))
      (my/pycell--delete (my/pycell--overlay event)))

    (defun my/pycell-copy-output (&optional event)
      "Copy the result of the cell at point, or the one clicked in EVENT.
    Text properties are kept, so images survive yanking — as they do when
    copying from the popup buffer."
      (interactive (list last-input-event))
      (kill-new (cadr (overlay-get (my/pycell--overlay event) 'my/pycell)))
      (message "my/pycell: result copied"))

    (defun my/pycell--image (text)
      "Return the first image displayed in TEXT, or nil."
      (let ((pos 0) img)
        (while (and (not img)
    ...
    (defun my/pycell-save-image (&optional event)
      "Save the first image of the result at point, or the one clicked in EVENT.
    The file type comes from the image descriptor: `create-image' detected
    it from the data's magic bytes when the image was rendered."
      (interactive (list last-input-event))
      (let* ((text (cadr (overlay-get (my/pycell--overlay event) 'my/pycell)))
             (img (or (my/pycell--image text)
        ...
        (message "my/pycell: image saved to %s" file)))

    (defun my/pycell-pop-output (&optional event)
      "Show the result of the cell at point, or the one clicked in EVENT, in a buffer.
    Each cell gets its own buffer, so results can be viewed side by side."
      (interactive (list last-input-event))
      (let* ((ov (my/pycell--overlay event))
             (text (cadr (overlay-get ov 'my/pycell)))
    ...
    (defvar my/pycell--queue nil
      "Start positions of the cells `my/pycell-restart-and-run-all' still has to run.")

    (defvar-local my/pycell--target nil
      "State of the cell running in this Python shell: (FROM BEG END TAIL START).
    FROM marks where its output starts here, BEG and END delimit the cell in
    its own buffer, TAIL accumulates the last output chunks and START is the
    `float-time' the cell was sent.")

    (defvar-local my/pycell--ticker nil
      "Timer driving the stopwatch of this shell's running cell.
    Buffer-local, so several (project-dedicated) shells do not interfere.")

    (defun my/pycell--quiet-erase (buf from bound)
      "Erase a finished cell's output between FROM and BOUND in shell BUF."
      (when (buffer-live-p buf)
        (with-current-buffer buf
          (let ((inhibit-read-only t))
            (delete-region from bound))
          (set-marker from nil)
          (set-marker bound nil))))

    (defun my/pycell--died ()
      "Finalize the running cell as aborted — its prompt will never return.
    Appends a death notice, with the exit status when available, to the
    output received so far.  Call in the Python shell buffer with
    `my/pycell--target' set."
      (pcase-let ((`(,from ,beg ,fin ,_ ,start) my/pycell--target)
                  (proc (get-buffer-process (current-buffer))))
        (setq my/pycell--target nil
              my/pycell--queue nil)
        (remove-hook 'comint-output-filter-functions #'my/pycell--filter t)
        (when (timerp my/pycell--ticker) (cancel-timer my/pycell--ticker))
        (when (buffer-live-p (marker-buffer beg))
          (let* ((out (my/pycell--clean
                       (replace-regexp-in-string
                 ...
                 (msg (propertize
                       (format "Process unexpectedly died%s"
                               (if proc
                                 ...
                                 ""))
                       'face 'error))
                 (text (if (string-empty-p out) msg (concat out "\n" msg))))
            (with-current-buffer (marker-buffer beg)
              (my/pycell--show beg fin text (- (float-time) start) 'died))))))

    (defun my/pycell--shell-reset ()
      "Finalize the running cell when its shell goes away under it.
    On `kill-buffer-hook' and `change-major-mode-hook' in the Python
    shell: the ticker alone cannot handle either case — a killed buffer's
    local state is unreachable, and `python-shell-restart' reinitializes
    the mode, wiping the local state before the next tick sees the dead
    process."
      (when my/pycell--target (my/pycell--died)))

    (defun my/pycell--tick (buf timer)
      "Mirror the running cell's output and stopwatch into its overlay.
    Runs in TIMER every 0.2s for the Python shell BUF; self-cancels when
    nothing is running there anymore, and finalizes the cell as aborted
    when the interpreter died mid-cell."
      (if (not (and (buffer-live-p buf)
                    (buffer-local-value 'my/pycell--target buf)))
          (cancel-timer timer)
        (with-current-buffer buf
          (if (not (process-live-p (get-buffer-process buf)))
              (progn (my/pycell--died) (cancel-timer timer))
            (pcase-let ((`(,from ,beg ,fin ,_ ,start) my/pycell--target))
              ;; Strip a trailing, still incomplete escape sequence:
              ;; comint-mime renders it only once it is complete.
              (let ((text (my/pycell--clean
                           (replace-regexp-in-string
                            "\e\\][^\e]*\\'" ""
                            (buffer-substring from (point-max))))))
                (when (buffer-live-p (marker-buffer beg))
                  (with-current-buffer (marker-buffer beg)
                    (my/pycell--show beg fin text (- (float-time) start)
                                     t)))))))))

    (defun my/pycell--filter (output)
      "Detect the running cell's completion and show its final result.
    Runs in the Python shell, from `comint-output-filter-functions'.  The
    intermediate output needs no handling here: the stopwatch ticker
    mirrors it into the overlay every fraction of a second anyway."
      (when my/pycell--target
        (pcase-let ((`(,from ,beg ,fin ,tail ,start) my/pycell--target))
          ;; A chunk boundary can split the prompt, so match the recent tail.
          (setq tail (concat tail (ansi-color-filter-apply output)))
          (setf (nth 3 my/pycell--target)
                (substring tail (max 0 (- (length tail) 256))))
          (when (python-shell-comint-end-of-output-p tail)
            (setq my/pycell--target nil)
            (remove-hook 'comint-output-filter-functions #'my/pycell--filter t)
            (let* ((bound (if comint-last-prompt
                              (car comint-last-prompt)
                            (point-max)))
                   (text (my/pycell--clean (buffer-substring from bound))))
              (when my/pycell-quiet-repl
                ;; Erase on a timer, not here: comint still works on this
                ;; region (scrolling, marker bookkeeping) right after
                ;; these hooks ran.
                (run-with-timer 0 nil #'my/pycell--quiet-erase
                                (current-buffer)
                                (copy-marker from) (copy-marker bound)))
              (when (buffer-live-p (marker-buffer beg))
                (with-current-buffer (marker-buffer beg)
                  (my/pycell--show beg fin text (- (float-time) start))))
              ;; Keep `my/pycell-restart-and-run-all' going, or stop on error.
              (when my/pycell--queue
                (if (string-match-p "Traceback (most recent call last)" text)
                    (progn (setq my/pycell--queue nil)
                           (message "my/pycell: stopped at error"))
                  (my/pycell--run-next))))))))

    (defun my/pycell-eval-region (start end)
      "Send START..END to the Python shell and mirror the output below the cell.
    Matches the calling convention of `code-cells-eval-region-commands'.
    Without a running interpreter one is started, and the cell evaluates
    as soon as its first prompt arrives."
      ;; ponytail: one cell at a time; add a queue if that ever chafes.
      (if (not (python-shell-get-process))
          ;; Cold start: queue this one cell — `my/pycell--run-next'
          ;; evaluates it once the fresh interpreter prompts.
          (progn
            (push (copy-marker start) my/pycell--queue)
            ...
            (with-current-buffer
              ...
              (add-hook 'python-shell-first-prompt-hook
                        #'my/pycell--run-next 90 t))
            (message "my/pycell: starting the interpreter…"))
        (my/pycell--eval-region-1 start end)))

    (defun my/pycell--eval-region-1 (start end)
      "Send START..END to the running Python shell and mirror its output."
      (let ((proc (python-shell-get-process-or-error))
            (beg (copy-marker start))
            (fin (copy-marker end t)))
        (with-current-buffer (process-buffer proc)
          (when my/pycell--target
            (user-error "The Python shell is still busy with another cell"))
          (setq my/pycell--target
                (list (copy-marker (process-mark proc)) beg fin "" (float-time)))
          ;; comint-mime renders from the same hook: append, so that by the time
          ;; we copy the region it already carries images instead of OSC escapes.
          (add-hook 'comint-output-filter-functions #'my/pycell--filter t t)
          (add-hook 'kill-buffer-hook #'my/pycell--shell-reset nil t)
          (add-hook 'change-major-mode-hook #'my/pycell--shell-reset nil t)
          (when (timerp my/pycell--ticker) (cancel-timer my/pycell--ticker))
          ;; The timer receives itself, so it can always self-cancel — even
          ;; after a shell restart wiped this buffer-local variable.
          (setq my/pycell--ticker (run-with-timer 0.2 0.2 #'ignore))
          (timer-set-function my/pycell--ticker #'my/pycell--tick
                              (list (current-buffer) my/pycell--ticker)))
        (my/pycell--show beg fin "" 0.0 t)
        ;; `python-shell-send-region' pads the code so that traceback line
        ;; numbers match the buffer.
        (python-shell-send-region start end)))

    (defun my/pycell-interrupt ()
      "Send a KeyboardInterrupt to the cell's Python process.
    The interrupted cell finishes normally: IPython prints the
    KeyboardInterrupt traceback and prompts again."
      (interactive)
      (interrupt-process (python-shell-get-process-or-error)))

    (defun my/pycell-restart ()
      "Restart the Python interpreter and discard all results."
      (interactive)
      (my/pycell-remove-overlays)
      (setq my/pycell--queue nil)
      (if-let* ((proc (python-shell-get-process)))
          (progn
            (with-current-buffer (process-buffer proc)
              ;; Cancel the ticker before the restart wipes the local
              ;; variable holding it, and drop the target so the abort
              ;; handler does not resurrect the running cell's overlay —
              ;; this command's contract is to discard all results.
              (when (timerp my/pycell--ticker) (cancel-timer my/pycell--ticker))
              (setq my/pycell--target nil))
            (python-shell-restart))
          (run-python nil python-shell-dedicated)))

    (defun my/pycell--run-next ()
      "Evaluate the cell at the head of `my/pycell--queue'."
      (remove-hook 'python-shell-first-prompt-hook #'my/pycell--run-next t)
      (when-let* ((m (pop my/pycell--queue)))
        (if (not (buffer-live-p (marker-buffer m)))
            (setq my/pycell--queue nil)
          (with-current-buffer (marker-buffer m)
            (goto-char m)
            (pcase-let ((`(,beg ,end) (code-cells--bounds nil nil t)))
              (my/pycell-eval-region beg end))))))

    (defun my/pycell-stop ()
      "Stop `my/pycell-restart-and-run-all' after the current cell."
      (interactive)
      (setq my/pycell--queue nil)
      (message "my/pycell: run all stopped"))

    (defvar-keymap my/pycell-stop-map
      :doc "Transient keymap active while all cells are being evaluated."
      "<escape>" #'my/pycell-stop)

    (defun my/pycell-restart-and-run-all ()
      "Restart the Python interpreter, then evaluate every cell in order.
    Stops at the first error, or on \\<my/pycell-stop-map>\\[my/pycell-stop]."
      (interactive)
      (my/pycell-restart)
      (setq my/pycell--queue
            (save-excursion
              (goto-char (point-min))
              (let ((cells (unless (looking-at-p code-cells-boundary-regexp)
                ...
                (nreverse cells))))
      ;; Evaluation may only start once the fresh interpreter has prompted —
      ;; and after comint-mime's setup, which runs off the same hook, hence
      ;; the depth.  `my/pycell--filter' then chains the remaining cells.
      (with-current-buffer (process-buffer (python-shell-get-process-or-error))
        (add-hook 'python-shell-first-prompt-hook #'my/pycell--run-next 90 t))
      (set-transient-map my/pycell-stop-map (lambda () my/pycell--queue) nil
                         "Evaluating all cells, %k to stop"))

    (define-minor-mode my/pycell-mode
      "Show Python cell results inline, below their cells.
    Cell evaluation is routed through `my/pycell-eval-region' while this
    mode is on; disabling it removes all results and falls back to plain
    `python-shell-send-region'.  All bindings live in the code-cells maps."
      ;; The :lighter also keeps the body from being taken for the
      ;; deprecated positional INIT-VALUE argument.
      :lighter " pycell"
      (unless my/pycell-mode
        (my/pycell-remove-overlays)))

    (defun my/pycell-mode-maybe ()
      "Enable `my/pycell-mode' in Python cell buffers."
      (when (derived-mode-p 'python-base-mode)
        (my/pycell-mode)))

    :hook
    (code-cells-mode . my/pycell-mode-maybe)

    :config
    ;; Keyed on the minor mode: with it off, code-cells falls through to its
    ;; stock python entry, `python-shell-send-region'.
    (with-eval-after-load 'code-cells
      (setf (alist-get 'my/pycell-mode code-cells-eval-region-commands)
            #'my/pycell-eval-region))

    (with-eval-after-load 'auto-side-windows
      (add-to-list 'auto-side-windows-right-buffer-names "^\\*pycell: .*\\*$")))
#+end_src
