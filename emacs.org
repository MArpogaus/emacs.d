...
#+begin_src emacs-lisp
  (use-package mood-line
    :after tab-bar
    :config
    (setq my/modeline-height my/tab-bar-tab-height)
    :custom
  ...
  (use-package tab-bar
    ...
    TAB is the tab object. I is the tab index. CURRENT-P indicates if the tab is currently selected."
      (let* ((tab-group-name (funcall tab-bar-tab-group-function tab))
             ...
             ;;                                        :foreground color
             ;;                                        :background color)))
             (group-sep (my/get-bar-image my/tab-bar-tab-height (if current-p 4 2) color))
             (group-icon (cond
...
#+begin_src emacs-lisp
  (use-package denote
    ...
    (defun my/denote-tab-group-hook ()
      "Hook function to run after creating a new note with Denote."
      (setq-local tab-bar-new-tab-choice t)
      (tab-bar-new-tab)
      (tab-group "[N] Denote"))
    :init
    ...
    ((text-mode . denote-fontify-links-mode-maybe)
     ...
     (denote-after-new-note . my/denote-tab-group-hook)))
#+end_src
...
#+begin_src emacs-lisp
  (use-package denote-menu
    :after denote
    :config
    (advice-add #'list-denotes :after #'my/denote-tab-group-hook)
    :bind
    (:map my/denote-map
          ("m" . list-denotes)))
#+end_src
