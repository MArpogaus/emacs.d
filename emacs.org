...
`create-image' derived from the data's magic bytes).

The moving parts are deliberately few: per shell, one buffer-local
=my/pycell--run= holds the running cell's entire state, a filter on
=comint-output-filter-functions= watches for the closing prompt, and a
0.2s ticker mirrors output, stopwatch and interpreter health.  Every way
a cell can end — prompt, dead process, killed shell buffer, shell
restart (the latter two via =my/pycell--abort= on the shell's kill and
major-mode hooks) — funnels through the single exit =my/pycell--end=.
Evaluation never blocks Emacs, but one cell at a time: sending while
another cell is still running is refused; without an interpreter, one is
started and the cell follows on its first prompt.  In the cells repeat
map, =O=
...
#+begin_src emacs-lisp
  (use-package emacs
    ...
    (defvar-local my/pycell--run nil
      "State of the cell running in this Python shell, or nil.
    A list (FROM BEG END TAIL START TIMER): FROM marks where the cell's
    output starts here, BEG and END delimit the cell in its own buffer,
    TAIL accumulates recent output for the prompt detection, START is the
    `float-time' the cell was sent and TIMER its ticker.")

    (defvar-local my/pycell--cold-cell nil
      "Cell (BEG . END markers) waiting for this shell's first prompt.")

    (defun my/pycell--output-so-far (from)
      "Return the running cell's output inserted after FROM, cleaned.
    A trailing, still incomplete escape sequence is dropped: comint-mime
    renders it only once it is complete."
      (my/pycell--clean
       (replace-regexp-in-string
        "\e\\][^\e]*\\'" "" (buffer-substring from (point-max)))))

    (defun my/pycell--end (text &optional died)
      "End the running cell, showing TEXT as its final result.
    The one exit for every way a cell ends; DIED marks abnormal ends.
    Call in the Python shell buffer."
      (pcase-let ((`(,_ ,beg ,fin ,_ ,start ,timer) my/pycell--run))
        (setq my/pycell--run nil)
        (cancel-timer timer)
        (when died (setq my/pycell--queue nil))
    ...
    output received so far.  Covers a dead interpreter (found by the
    ticker), a killed shell buffer and a shell restart, which
    reinitializes the major mode — hence also on `kill-buffer-hook' and
    `change-major-mode-hook' in the Python shell."
      (when my/pycell--run
        (let* ((proc (get-buffer-process (current-buffer)))
               (out (my/pycell--output-so-far (car my/pycell--run)))
               (msg (propertize
                     (format "Process unexpectedly died%s"
                             (if proc
                               ...
                               ""))
                     'face 'error)))
          (my/pycell--end (if (string-empty-p out) msg (concat out "\n" msg))
                          t))))
    ...
    nothing is running there anymore."
      (if (not (and (buffer-live-p buf)
                    (buffer-local-value 'my/pycell--run buf)))
          (cancel-timer timer)
        (with-current-buffer buf
          (if (not (process-live-p (get-buffer-process buf)))
              (my/pycell--abort)
            (pcase-let ((`(,from ,beg ,fin ,_ ,start ,_) my/pycell--run))
              (let ((text (my/pycell--output-so-far from)))
                (when (buffer-live-p (marker-buffer beg))
                  (with-current-buffer (marker-buffer beg)
                    (my/pycell--show beg fin text (- (float-time) start)
                                     t)))))))))
    ...
    (defun my/pycell--filter (output)
      "Watch OUTPUT for the running cell's closing prompt, then end the cell.
    Stays on `comint-output-filter-functions' once installed and idles
    while no cell runs; the live mirroring is the ticker's job."
      (when my/pycell--run
        ;; A chunk boundary can split the prompt, so match against a capped
        ;; tail; `ansi-color-filter-apply' drops the escape sequences.
        (let ((tail (concat (nth 3 my/pycell--run)
                            (ansi-color-filter-apply output))))
          (setf (nth 3 my/pycell--run)
                (substring tail (max 0 (- (length tail) 256))))
          (when (python-shell-comint-end-of-output-p tail)
            (my/pycell--end
             (my/pycell--clean
              (buffer-substring (car my/pycell--run)
                                (if comint-last-prompt
                                    (car comint-last-prompt)
                                  (point-max)))))))))

    (defun my/pycell--send (proc start end)
      "Send START..END to PROC as the running cell and track it.
    Call with the cell's buffer current."
      (let ((beg (copy-marker start))
            (fin (copy-marker end t)))
        (with-current-buffer (process-buffer proc)
          (when my/pycell--run
            (user-error "The Python shell is still busy with another cell"))
          ;; All idempotent: the filter idles while no cell runs, the other
          ;; two catch the shell going away under a running cell.  comint-mime
          ;; renders from the same hook; appending means the copied region
          ;; already carries images instead of OSC escape sequences.
          (add-hook 'comint-output-filter-functions #'my/pycell--filter t t)
          (add-hook 'kill-buffer-hook #'my/pycell--abort nil t)
          (add-hook 'change-major-mode-hook #'my/pycell--abort nil t)
          ;; The ticker receives itself, so it can always self-cancel.
          (let ((timer (run-with-timer 0.2 0.2 #'ignore)))
            (timer-set-function timer #'my/pycell--tick
                                (list (current-buffer) timer))
            (setq my/pycell--run (list (copy-marker (process-mark proc))
    ...
    (defun my/pycell--run-cold ()
      "Evaluate the cell that waited for the interpreter to come up.
    Unlike `my/pycell--run-next' this must not move point: the command
    that triggered the cold start may have moved it on already."
      (remove-hook 'python-shell-first-prompt-hook #'my/pycell--run-cold t)
      (pcase-let ((`(,beg . ,end) my/pycell--cold-cell)
                  (proc (get-buffer-process (current-buffer))))
        (setq my/pycell--cold-cell nil)
        (when (and beg (buffer-live-p (marker-buffer beg)))
          (with-current-buffer (marker-buffer beg)
            (my/pycell--send proc beg end)))))

    (defun my/pycell-eval-region (start end)
      "Evaluate START..END as a cell, mirroring the output below it.
    ...
    as soon as its first prompt arrives."
      ...
      (if-let* ((proc (python-shell-get-process)))
          (my/pycell--send proc start end)
        (run-python)
        (with-current-buffer (process-buffer (python-shell-get-process-or-error))
          (setq my/pycell--cold-cell (cons (copy-marker start)
                                           (copy-marker end t)))
          (add-hook 'python-shell-first-prompt-hook #'my/pycell--run-cold 90 t))
    ...
    (defun my/pycell-restart ()
      ...
      (if-let* ((proc (python-shell-get-process)))
          (progn
            ;; Drop the run silently: this command discards results rather
            ;; than marking the running cell as aborted.  The orphaned
    ...
    (defun my/pycell--run-next ()
      "Evaluate the cell at the head of `my/pycell--queue', point following."
    ...
    Stops at the first error, or on \\<my/pycell-stop-map>\\[my/pycell-stop]."
      ...
      ;; the depth.  `my/pycell--end' then chains the remaining cells.
      (with-current-buffer (process-buffer (python-shell-get-process-or-error))
